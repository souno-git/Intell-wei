...
$query = mysql_query("SELECT * from driver WHERE driver_id='$username'");
...
if($exists > 0) //如果没有返回或者用户不存在
{
    ...
    {
        $table_users = $row['driver_id']; // 逐行对比所有相同的用户直到完成
        $table_password = $row['dpassword'];
    ...
    if(($username == $table_users) && ($password == $table_password)) // 检查是否有任何匹配字段
    {
        ...
        {
            $_SESSION['duser'] = $username;//在一个session中设置用户名。这是一个全局变量
...
{
    Print '<script charset="utf-8">alert("驾驶证号错误！");</script>';
